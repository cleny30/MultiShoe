...
export default function Edit({
  ...
  const UpdateAddress = async () => {
    ...
    if (flag) {
      const response = await doRequest(
        "post",
        "api/DeliveryAddress/UpdateAddress",
        {
          data: {
            id: idToUpdate,
            userName: userName,
  ...
  useEffect(() => {
    const data = deliveryAddress.find((address) => address.id === idToUpdate);
    setFullName(data.fullname);
    setPhoneNumber(data.phoneNumber);
    setOriginalAddress(data.address);
    setOriginalDefault(data.isDefault);
    setSpecificAddress(data.specificAddress);
    setIsDefault(data.isDefault);
    getDataProvinces();
    var split = data.address.split(",");
    setProvince(split[0]);
    setDistrict(split[1]);
    setWard(split[2]);
  }, [idToUpdate]);
      ...
      <div className="fixed top-0 left-0 bg-[rgba(0,0,0,.3)] h-[100%] w-[100%] z-[100]">
        <div className="update-review fixed bg-slate-50 top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-30 w-[30%]">
          <div className="flex px-6 py-3 border-solid border-b-2">
            <span className="text-[20px] font-bold">Update address</span>
          </div>
          <div className="block pt-0 px-6 pb-6 h-[350px]">
            ...
            <div className="flex mt-3 relative">
              ...
              >
                <input
                  ...
                  className={`before:content[''] peer relative h-5 w-5 cursor-pointer appearance-none rounded-md border border-blue-gray-200 transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-[#ffc43d] checked:bg-[#ffc43d] checked:before:bg-[#ffc43d] hover:before:opacity-10 
                      ${originalDefault ? "!cursor-not-allowed" : ""}`}
                  id="red"
                  checked={isDefault}
                  disabled={originalDefault}
              ...
              {showNotification && (
                <div className="absolute bottom-9 left-1">
                  <div className="box-address box--bottom ">
                    You cannot remove the <strong>"Default address"</strong>{" "}
                    label. Please set another address as your primary address.
                  </div>
                </div>
              )}
          ...
          <div className="flex bg-white border-t-2 border-solid px-6 h-16 justify-end">
            <Button
              ...
              handleClickBtn={() => closeUpdateAddressForm()}
            />
            <Button
              text="Submit"
              className="!px-4 h-10"
              handleClickBtn={() => UpdateAddress()}
            />
          </div>
        </div>
      </div>
