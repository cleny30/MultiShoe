...
import { fetchFavorites } from '../../pages/FavoriteProduct/favoriteSlice'; // Thay đổi đường dẫn đến slice của Redux
...
export default function Shop() {
  ...
  async function getData() {
    const response = await doRequest("get", "api/Shop");
    setRes(response.data.result);
    await dispatch(fetchFavorites(userName)); 
  }

  const handleSetPro = ()=>{
    setPro(favorite.slice(startIndex, endIndex));
  }
  ...
  useEffect(() => {
    ...
    setTotalPage(Math.ceil(favorite.length / itemsPerPage));
    handleSetPro();
    filterProduct();
  }, [category, brand, sort, price, order, currentPage, itemsPerPage, dispatch, favorite]);
  return (
    <div className="shop">
      {res.length !== 0 && (
        <>
          <Header res={res} userName={userName} productList={res.product} />
          {/* <!-- Breadcrumb Start --> */}
            ...
            <div className="row px-xl-5">
              <div className="col-12">
                <nav className="breadcrumb bg-light mb-30">
                  ...
                  <span className="breadcrumb-item active">Favorite Product</span>
                </nav>
              </div>
            </div>
          </div>
