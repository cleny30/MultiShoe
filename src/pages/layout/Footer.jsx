...
import { CONTACTINFO, QUICKSHOPLINKS, MYACCOUNTLINK } from '../../constants/common';
            ...
            <div className="container-fluid bg-dark text-secondary mt-5 pt-5">
                <div className="row px-xl-5 pt-5">
                    <div className="col-lg-4 col-md-12 mb-5 pr-3 pr-xl-5">
                        ...
                        <p className="mb-2"><i className="fa fa-map-marker-alt text-primary mr-3"></i>{CONTACTINFO.ADDRESS}</p>
                        <p className="mb-2"><i className="fa fa-envelope text-primary mr-3"></i>{CONTACTINFO.EMAIL}</p>
                        <p className="mb-0"><i className="fa fa-phone-alt text-primary mr-3"></i>{CONTACTINFO.PHONE}</p>
                    </div>
                        ...
                        <div className="row">
                            <div className="col-md-4 mb-5">
                                ...
                                <div className="d-flex flex-column justify-content-start">
                                    {QUICKSHOPLINKS.map((link, index) => (
                                        <Link key={'item-' + index} className="text-secondary mb-2" to={link.PATH}><i className="fa fa-angle-right mr-2"></i>{link.LABEL}</Link>
                                    ))}
                                </div>
                            </div>
                            <div className="col-md-4 mb-5">
                                ...
                                <div className="d-flex flex-column justify-content-start">
                                    {userName == null ? (
                                        MYACCOUNTLINK.UNLOG.map((link, index) => (
                                            <Link key={'item-' + index} className="text-secondary mb-2" to={link.PATH}><i className="fa fa-angle-right mr-2"></i>{link.LABEL}</Link>
                                        ))
                                    ) : (
                                        MYACCOUNTLINK.LOGED.map((link, index) => (
                                            <Link key={'item-' + index} className="text-secondary mb-2" to={link.PATH} onClick={link.PATH === "/" ? handleLogout : undefined}><i className="fa fa-angle-right mr-2"></i>{link.LABEL}</Link>
                                        ))
                                    )}
                                </div>
                            </div>
                ...
                <div className="row border-top mx-xl-5 py-4 footer-domain">
